...
export const getTrackRecommendations = async (
  ...
  try {
    // Get track analysis (ensure it's valid)
    const trackAnalysis = await getTrackAnalysis(session, trackId);
    if (!trackAnalysis || typeof trackAnalysis !== 'object') {
      console.error('Invalid track analysis data', trackAnalysis);
      return []; // Or handle this case as needed
    }

    // Define the track features
    const trackFeatures = {
      acousticness: 1,
      danceability: 1,
      energy: 1,
    ...
    // Build the recommendations API endpoint
    ...
    if (!data || !data.tracks) {
      console.error('No track recommendations returned or invalid data');
      return []; // Return an empty array if there's no valid data
    }

    return data.tracks; // Return the list of track recommendations
  } catch (error) {
